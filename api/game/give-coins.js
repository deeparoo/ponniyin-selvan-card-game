...
const { deserializeG, serializeG, applyGiveCoins } = require('../_lib/gameLogic');
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (!roomCode || !playerId || amount == null) {
      return res.status(400).json({ error: 'roomCode, playerId, and amount are required' });
    }
    ...
    if (G.phase !== 'GIVE_COINS_SELECT') {
      return res.status(400).json({ error: `Cannot give coins in phase: ${G.phase}` });
    }

    G = applyGiveCoins(G, seatIndex, parseInt(amount, 10));
    ...
    return res.status(500).json({ error: 'Failed to give coins' });
  }
};
