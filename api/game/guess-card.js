...
const { deserializeG, serializeG, applyGuessCard } = require('../_lib/gameLogic');
...
module.exports = async (req, res) => {
  ...
  try {
    const { roomCode, playerId, guessedCharId } = req.body || {};
    if (!roomCode || !playerId || !guessedCharId) {
      return res.status(400).json({ error: 'roomCode, playerId, and guessedCharId are required' });
    }
    ...
    if (G.phase !== 'GUESS_SELECT') {
      return res.status(400).json({ error: `Cannot guess in phase: ${G.phase}` });
    }

    G = applyGuessCard(G, seatIndex, guessedCharId);
    ...
    console.error('guess-card error:', err);
    return res.status(500).json({ error: 'Failed to guess card' });
  }
};
