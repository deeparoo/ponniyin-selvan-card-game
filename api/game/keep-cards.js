...
const { deserializeG, serializeG, applyKeepCards } = require('../_lib/gameLogic');
...
module.exports = async (req, res) => {
  ...
  try {
    const { roomCode, playerId, keepUids } = req.body || {};
    if (!roomCode || !playerId || !Array.isArray(keepUids)) {
      return res.status(400).json({ error: 'roomCode, playerId, and keepUids (array) are required' });
    }
    ...
    if (G.phase !== 'CARD_EXCHANGE_SELECT') {
      return res.status(400).json({ error: `Cannot keep cards in phase: ${G.phase}` });
    }

    G = applyKeepCards(G, seatIndex, keepUids);
    ...
    console.error('keep-cards error:', err);
    return res.status(500).json({ error: 'Failed to keep cards' });
  }
};
