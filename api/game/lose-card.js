...
const { deserializeG, serializeG, applyLoseCard } = require('../_lib/gameLogic');
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (G.phase !== 'LOSE_CARD_SELECT') {
      return res.status(400).json({ error: `Cannot lose card in phase: ${G.phase}` });
    }

    G = applyLoseCard(G, seatIndex, cardUid);
    ...
    console.error('lose-card error:', err);
    return res.status(500).json({ error: 'Failed to lose card' });
  }
};
